...
export const updateCommentVotes = (
  commentId: number,
  postId: number, // 追加: どのポストに属するコメントかを追跡
  upVotes: number,
  downVotes: number,
  ...
  mostVotedSetter((prev: MostVotedCommentsMap) => {
    const currentMostVoted = prev[postId] || { commentId: null, votes: 0 };

    // このコメントが現在の最多投票コメントより多い場合、または
    // このコメントが現在の最多投票コメントと同じで投票数が変わった場合
    if (
      totalVotes > currentMostVoted.votes ||
      (commentId === currentMostVoted.commentId &&
        totalVotes !== currentMostVoted.votes)
    ) {
      return {
        ...prev,
