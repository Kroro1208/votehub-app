...
export const createNotification = async (params: NotificationCreateParams) => {
  const { data, error } = await supabase.rpc("create_notification_secure", {
    ...
    p_post_id: params.post_id || null,
    p_nested_post_id: params.nested_post_id || null,
  });
...
export const notifyNestedPostTargets = async (
  ...
  try {
    // RPC関数を使用して安全に派生質問通知を一括作成
    const { data: notificationCount, error } = await supabase.rpc(
      "create_nested_post_notifications",
      {
        p_parent_post_id: parentPostId,
        p_nested_post_id: nestedPostId,
        p_nested_post_title: nestedPostTitle,
        p_target_vote_choice: targetVoteChoice,
        p_creator_user_id: creatorUserId,
      },
    );

    if (error) {
      throw new Error(`派生質問通知の作成に失敗しました: ${error.message}`);
    }

    if (notificationCount > 0) {
      console.log(`${notificationCount}人のユーザーに通知を送信しました`);
    } else {
