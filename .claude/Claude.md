...
#### ネスト投稿機能 (**GitHub Issue #12 - 部分実装済み**)
...
- **投票者ターゲット機能**: 賛成者向け/反対者向け/全員向けの絞り込み**⚠️部分実装**
  - ✅ UI選択機能・データベーススキーマ実装済み
  - ❌ 投票制限ロジック未実装（Issue #12の核心機能）
...
#### **Issue #12**: "派生質問の回答者の絞り込み" (**部分実装済み - 投票制限ロジック未完成**)

- **実装状況**: 90%完了
- **残作業**: target_vote_choiceに基づく投票制限ロジック実装
- **優先度**: 高 (ネスト投稿の核心機能)

#### **Issue #13**: "ユーザーに付与するポイント機能の作成" (**未実装**)
...
- **Issue #11**: ユーザープロフィールページ (2025-06-18クローズ)
- **Issue #10**: ランキングタイトルリンク (2025-06-16クローズ)
- **Issue #9**: スペース人気投稿表示 (2025-06-17クローズ)
- **Issue #8**: 投稿入力形式実装 (2025-06-17クローズ)
...
1. **ユーザーポイントシステム** (**GitHub Issue #13**)
...
2. **コメント共感・説得力システム** (**GitHub Issue #2**)
