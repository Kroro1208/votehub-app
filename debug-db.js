...
async function debugDatabase() {
  ...
  try {
    ...
    if (error) {
      ...
      if (data.length > 0) {
        console.log('   - データ例 (完全版):', JSON.stringify(data[0], null, 2));
        
        // 各フィールドが正しく含まれているかチェック
        const sampleData = data[0];
        const requiredFields = ['id', 'title', 'content', 'created_at', 'image_url', 'avatar_url', 'vote_count', 'comment_count', 'vote_deadline', 'community_id', 'user_id', 'parent_post_id', 'nest_level', 'target_vote_choice'];
        
        console.log('   - フィールドチェック:');
        requiredFields.forEach(field => {
          const exists = field in sampleData;
          const value = sampleData[field];
          console.log(`     ${exists ? '✅' : '❌'} ${field}: ${exists ? (value !== null ? value : 'null') : 'missing'}`);
        });
      }
    }
  } catch (err) {
  ...
  try {
    ...
    const { error: funcError } = await supabase.rpc('exec', { sql: updateFunction });
    
    if (funcError) {
      console.log('❌ 関数更新エラー:', funcError.message);
      
      // 別の方法で関数を更新
      console.log('⚠️  SQL実行権限がない可能性があります。代替案を確認します。');
    } else {
