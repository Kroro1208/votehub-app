-- Apply all notification fixes in the correct order
-- This script combines all necessary fixes for notification system

-- Step 1: Fix post deletion constraint
-- Drop the foreign key constraint that prevents post deletion
ALTER TABLE notification_logs 
DROP CONSTRAINT IF EXISTS notification_logs_post_id_fkey;
...
-- Step 2: Create all required functions for notification system
...
-- Step 3: Improve the nested post notification trigger with proper type handling
...
-- Step 4: Recreate the trigger to ensure it uses the updated function
...
-- Step 5: Create notification_logs table if it doesn't exist
...
GRANT EXECUTE ON FUNCTION check_deadline_notification_not_sent(INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION should_check_deadline_notification(INTEGER) TO authenticated;
...
-- Success message
SELECT 'All notification fixes applied successfully!' as status;